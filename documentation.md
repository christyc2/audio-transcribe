...
## Project Overview
This will be a fullstack audio transcription application.
...
### Axios-Based Architecture (Previous): 
    - `client.ts` creates an Axios instance (with the base URL of the API) and registers request/response 
    interceptors (similar to middleware). Also provides `setAccessToken` helper function to update/clear the 
    stored token and an `AuthError` class for authentication errors.
    - `auth.ts` redirects frontend registration and login requests to the corresponding api endpoints. It 
    also provides `fetchProfile` helper function to fetch user credentials (note: user authentication is 
    checked before calling `fetchProfile`).
    - `jobs.ts` provides helper functions to interact with the jobs API endpoints. `uploadJob` uploads a new 
    job to the backend and `fetchUserJobs` fetches the user's list of jobs.
    - Because React components only run in the browser, so it must send a POST request to FastAPI endpoints 
    (e.g., for registration), we use the Axios library. The Axios instance simplifies handling HTTP requests 
    and response for all components. The Axios instance takes care of attaching the base API URL, JSON 
    headers, error handling, and parsing payload into POST request.

### SDK-Based Architecture (Current): 
The frontend now uses an auto-generated TypeScript SDK created from the FastAPI OpenAPI schema. This provides type safety, autocompletion, and automatic synchronization with backend API changes.
    - Keeps frontend–backend API contracts synchronized
        - API contracts: the agreed-upon structure of requests and responses between frontend and backend. It defines what endpoints exist (/jobs, /login), what data is sent (JSON fields), what data is returned, what errors look like.
    - `frontend/src/api/sdk/` - Auto-generated SDK directory (gitignored) containing TypeScript client code generated by OpenAPI Generator
    - `frontend/src/api/sdkClient.ts` - Wrapper around the generated SDK that configures authentication, error handling, and provides convenient API access. Manages token injection and converts SDK errors to user-friendly messages.
    - `auth.ts` - Authentication API functions (register, login, fetchProfile) using the generated SDK instead of direct axios calls
    - `jobs.ts` - Job management API functions (uploadJob, fetchUserJobs) using the generated SDK
    - `client.ts` - Backward compatibility exports (AuthError class, setAccessToken re-export) for existing code
    Note: The above code acts as a wrapper/abstraction layer that:
        - Provides a consistent interface even if SDK method names change
        - Adds error handling and user-friendly messages
        - Manages authentication token lifecycle
        - Maintains backward compatibility with existing code
    - **SDK Generation**: The SDK is generated using OpenAPI Generator CLI (`npm run generate-sdk`) which:
        - Fetches the OpenAPI schema from the running FastAPI server at `http://127.0.0.1:8000/openapi.json`
        - Generates TypeScript/axios client code with type-safe interfaces matching FastAPI Pydantic models
        - Creates API classes (AuthenticationApi, UsersApi) with methods for each endpoint
        - Automatically handles request/response serialization and type checking
    - **Benefits of SDK Approach**:
        - Type safety: TypeScript interfaces automatically match backend models (User, Job, Token)
        - Autocompletion: IDE provides method suggestions and parameter types
        - Automatic updates: Regenerating SDK after API changes ensures frontend stays in sync
        - Reduced boilerplate: No manual axios calls, URL construction, or type definitions
        - Error handling: Centralized error handling with user-friendly error messages
        - In production/collaboration across teams: standardized code allows for easy calls to api backend from frontend
    - **Migration from Axios**: Previously used direct axios calls with manual interceptors. The SDK approach eliminates the need for:
        - Manual axios instance configuration
        - Request/response interceptors for authentication
        - Manual type definitions for API responses
        - Manual URL path construction
...
3. Auth State Management (`state/authStore.ts`, `components/AuthProvider.tsx`)
    ...
    - authStore (`state/authStore.ts`): the store holds **observable** data IN ONE PLACE about the user, status, error, and access token so that any React components can access them and automatically re-render when a state changes (e.g., authentication status). `authStore` also defined actions that can be called by components, such as `login`, `logout`, `hydrate`, and `setUser`. The implementation of these actions in the auth store ensure that the brower's `localStorage`, the auth store, and the module-level access token variable are all updated accordingly when an action handler is called. In short, the auth store centalizes user authentication states and action handlers for the React components to use.
    ...
    - context: the context encapsulates all the authentication states (e.g., user, status, accesstoken, etc.) that later components may use. 
...
6. Dashboard Experience (`components/Dashboard.tsx`)
    ...
    - Requests a new job to be created in the backend each time a new file is uploaded via the helper function (from SDK).
...
7. Foreign Key
    ...
    - Migrated changes with Alembic

# Frontend UI Enhancement with Component Libraries
1. Radix UI Themes Integration
    - Radix UI Themes is a component library built on top of Radix UI Primitives, providing customizable React components with built-in styling and theming support
    - Import global CSS: `import '@radix-ui/themes/styles.css'` in `main.tsx` (application root)
    - Wrap application with `<Theme>` provider component
    - Components are imported from `@radix-ui/themes` package
2.  Components Used
    - `AlertDialog`: Confirmation dialogs (e.g., logout confirmation popup)
    - `Button`: Styled button components with variants (solid, soft) and color options
    - `Flex`: Layout component for flexible container arrangements
    - `Theme`: Provider component that wraps the application to enable Radix UI theming
3. Benefits
    - `AlertDialog` handles focus trapping and escape key handling without manual implementation.
    - Reduced development time -- pre-built components reduce the need to create complex UI patterns from scratch
    - Type safe -- TypeScript support with proper type definitions for all component props, enabling autocompletion and compile-time error checking
    - Flexible customization -- while providing default logic, components can be styled with custom className props or through the theme configuration, allowing integration with existing Tailwind CSS styles
    - Cross-browser Compatibility -- Radix UI handles browser differences and edge cases internally, reducing cross-browser testing and fixes
...
- Audio upload & job listing: upload constraints, backend validation, dashboard refresh cycle.
    Audio uploads trigger background transcription jobs
    HTTP request returns immediately
    Celery workers process jobs asynchronously
    Redis used as broker + result backend
    Faster Whisper model loaded once per worker
